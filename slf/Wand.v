...
(** This chapter presents additional Separation Logic operators:
    ...
    - it is key to formulate the "ramified frame rule", a more practical rule
    ...
    The "optional material" presents several equivalent definitions for the
    magic wand. *)
...
(** The heap predicate [\forall x, H] holds of a heap [h] that, for any possibly
...
(** The introduction rule in an entailement for [\forall] appears below. To
    prove that a heap satisfies [\forall x, J x], one must show that, for any
...
(** The elimination rule in an entailment for [\forall] appears below. Assuming
    a heap satisfies [\forall x, J x], one can derive that the same heap
...
(** The heap predicate [hor H1 H2] describes a heap that satisfies [H1] or
    satifies [H2] (possibly both). In other words, the heap predicate
    [hor H1 H2] lifts the disjunction operator [P1 \/ P2] from [Prop] to [hprop]
    .
...
(** The two introduction rules for [hor] asserts that if [H1] holds, then
...
(** The elimination rule asserts that if [hor H1 H2] holds, then one can perform
    a case analysis on whether it is [H1] or [H2] that holds. Concretely, to
    show that [hor H1 H2] entails a heap predicate [H3], one must show both that
...
(** **** Exercise: 2 stars, standard, especially useful (hor_comm)
    ...
    Prove that [hor] is a symmetric operator. Hint: exploit [if_neg] and
    [hprop_op_comm] (from chapter [Himpl]). *)
...
(** **** Exercise: 4 stars, standard, especially useful (hor_comm)
    ...
    shown below. Hint: to prove this equivalence, do not attempt to unfold
    [MList], but instead work using the equalities [MList_nil] and [MList_cons].
    *)
...
(** The heap predicate [hand H1 H2] describes a heap that satisfies [H1] and at
    the same time satifies [H2]. In other words, the heap predicate [hand H1 H2]
    lifts the disjunction operator [P1 /\ P2] from [Prop] to [hprop]. The heap
    predicate [hand] admits a direct definition as a function over heaps. *)

Definition hand' (H1 H2 : hprop) : hprop :=
  fun h => H1 h /\ H2 h.

(** An alternative definition leverages the [\forall] quantifier. The
    definition, shown below, reads as follows: "for any boolean value [b], if
    [b] is true then [H1] should hold, and if [b] is false then [H2] should
    hold". *)
...
(** **** Exercise: 1 star, standard, especially useful (hand_comm)
    ...
    Prove that [hand] is a symmetric operator. Hint: use [hprop_op_comm] and
    [rewrite if_neg], or a case analysis on the boolean value coming from [hand]
    . *)
...
(** The magic wand operation [H1 \-* H2], to be read "H1 wand H2", defines a
    heap predicate such that, if we extend it with [H1], we obtain [H2].
    Formally, the following entailment holds:

      H1 \* (H1 \-* H2) ==> H2.

    Intuitively, if one thinks of the star [H1 \* H2] as the addition [H1 + H2],
    then one can think of [H1 \-* H2] as the subtraction [-H1 + H2]. The
    entailment stated above essentially captures the idea that [(-H1 + H2) + H1]
    simplifies to [H2].
    ...
    from [h] and that satisfies [H1], the union of [h] and [h'] satisfies [H2].
    The operator [hwand], which implements the notation [H1 \-* H2], may thus be
    defined as follows. *)
...
(** The definition above is perfectly fine, however it is more practical to use
    ...
    other words, [H0] is such that [(H1 \* H0) ==> H2]. In the definition of
    [hwand] shown below, observe how [H0] is existentially quantified. *)
...
(** The magic wand is not so easy to make sense of, at first. Reading its
    ...
    speaking, think of [H0 = -H1+H2] and [H1+H0 = H2] being equivalent. *)
...
(** It turns out that the magic wand operator is uniquely defined by the
    equivalence [(H0 ==> H1 \-* H2) <-> (H1 \* H0 ==> H2)]. In other words, as
    we establish further on, any operator that satisfies the above equivalence
    is provably equal to [hwand]. *)
...
(** The right-to-left direction of the equivalence is an introduction rule: it
    ...
    a state [H0]. To establish that [H0] entails [H1 \-* H2], one has to show
    that the conjunction of [H0] and [H1] yields [H2]. *)
...
(** The left-to-right direction of the equivalence is an elimination rule: it
    tells what can be deduced from an entailment [H0 ==> (H1 \-* H2)]. What can
    be deduced from this entailment is that if [H0] is starred with [H1], then
    [H2] can be recovered. *)
...
(** This elimination rule can be equivalently reformulated in the following
    form, which makes clearer that [H1 \-* H2], when starred with [H1], yields
    [H2]. *)
...
(** We next present the most important properties of the magic wand operator.
    Thereafter, the tactic [xsimpl] is available, yet in a form that does not
    provide support for the magic wand. The actual [xsimpl] tactic would
    trivially solve many of these lemmas, but using it here would be cheating
    because the implementation of [xsimpl] itself relies on several of these
    lemmas. *)

(** The operator [H1 \-* H2] is contravariant in [H1] and covariant in [H2],
    similarly to the implication operator [->]. *)
...
(** Two predicates [H1 \-* H2] ans [H2 \-* H3] may simplify to [H1 \-* H3]. This
    simplification is reminiscent of the arithmetic operation
    [(-H1 + H2) + (-H2 + H3) = (-H1 + H3)]. *)
...
(** An interesting property is that arguments on the LHS of a magic wand can
    equivalently be "curried" or "uncurried", just like a function of type
    [(A *
    B) -> C] is equivalent to a function of type [A -> B -> C].

    The heap predicates [(H1 \* H2) \-* H3] and [H1 \-* (H2 \-* H3)] and
    [H2 \-* (H1 \-* H3)] are all equivalent. Intuitively, they all describe the
    predicate [H3] with the missing pieces [H1] and [H2].
...
(** Another interesting property is that the RHS of a magic wand can absorb
    ...
    Concretely, from [(H1 \-* H2) \* H3], one can get the predicate [H3] to be
...
(** The reciprocal entailment is false: [H1 \-* (H2 \* H3)] does not entail
    [(H1 \-* H2) \* H3]. To see why, instantiate [H1] with [\[False]]. The
    predicate [H1 \-* (H2 \* H3)] is equivalent to [True], hence imposes no
    restriction on the heap. Yet, to establish [(H1 \-* H2) \* H3], one would
    need to exhibit a piece of heap satisfiying [H3]. *)
...
(** In what follows, we generalize the magic wand to operate on postconditions,
    introducing a heap predicate of the form [Q1 \--* Q2], of type [hprop]. Note
    that the magic wand between two postconditions produces a heap predicate,
    and not a postcondition. There are two ways to define the operator [qwand].
    *)

(** The first possibility is to follow the same pattern as for [hwand], that is,
    to quantify some heap predicate [H0] such that [H0] starred with [Q1] yields
    [Q2]. *)
...
(** Fourht the operation [Q1 \--* Q2] can absorb in its RHS resources to which
    it is starred. *)
...
(** One can extend the tactic [xsimpl] to recognize the magic wand, and
    automatically perform a number of obvious simplifications. This extension is
    implemented in the file [LibSepSimpl], which exports the tactic [xsimpl]
    illustrated in this section. *)

Module XsimplDemo.

(** The tactic [xsimpl] is able to spot a magic wand that cancels out. For
    example, if an iterated separating conjunction includes both [H2 \-* H3] and
    [H2], then these two heap predicates can be simplified into [H3]. *)
...
(** This rule suffers from a practical issue, which we illustrate in details on
    ...
    In short, the problem stems from the fact that we need to instantiate [H2]
    for applying the rule. Providing [H2] by hand is not practical, thus we need
    to infer it. The value of [H2] can be computed as the subtraction of [H]
    minus [H1]. The resulting value may then exploited in the last premise for
    constructing [Q1 \*+ H2]. This transfer of information via [H2] from one
    subgoal to another can be obtained by introducing an "evar" (Coq unification
    variable) for [H2]. However this approach does not work well in the cases
    where [H] contains existential quantifiers. Indeed, such existential
    quantifiers are typically first extracted out of the entailment
    [H ==> H1 \* H2] by the tactic [xsimpl]. However, these existentially
    quantified variables are not in the scope of [H2], hence the instantiation
    of the evar associated with [H2] typically fails. *)

(** The "ramified frame rule" exploits the magic wand operator to circumvent the
    problem, by merging the two premises [H ==> H1 \* H2] and [Q1 \*+ H2 ===> Q]
    into a single premise that no longer mentions [H2]. This replacement premise
    is [H ==> H1 \* (Q1 \--* Q)]. To understand where it comes from, observe
    first that the second premise [Q1 \*+ H2 ===> Q] is equivalent to
    [H2 ==> (Q1 \--* Q)]. By replacing [H2] with [Q1 \--* Q] inside the first
    premise [H ==> H1 \* H2], we obtain the new premise
...
(** Earlier on, we sketched an argument claiming that the consequence-frame rule
    ...
    concrete example, and show how the ramified frame rule smoothly handles that
    same example. Recall, once again, the consequence-frame rule. *)
...
(** One practical caveat with this rule is that we must resolve [H2], which
    ...
    existential quantifier. For a concrete example, consider the specification
    of the function [ref], which allocates a reference. *)
...
(** Assume that wish to derive the following triple, which extends both the
    ...
    with the heap predicate [\exists l' v', l' ~~> v']. This predicate describes
...
(** As another tempting yet false property of the magic wand, consider the
    ...
    [H2 ==> H1 \* (H1 \-* H2)]. It does not hold in general. As counter-example,
    consider [H2 = \[]] and [H1 = \[False]]. The empty heap satisfies the
    left-hand side of the entailment, yet it does does not satisfy
    [\[False] \* (\[False] \-* \[])], because there is no way to establish
    [False] out of thin air. *)
