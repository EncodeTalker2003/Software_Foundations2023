...
Inductive bevalR: bexp -> bool -> Prop :=
  ...
  | E_BNot (b: bexp) (bv: bool) : b ==>b bv -> BNot b ==>b negb bv
  | E_BAnd (b1 b2: bexp) (bv1 bv2: bool) : b1 ==>b bv1 -> b2 ==>b bv2 -> BAnd b1 b2 ==>b andb bv1 bv2
...
Proof.
  ...
  - intros H. destruct b; subst; constructor;
...
Proof.
  ...
  - intros. induction c.
	...
	+ simpl in H. rewrite andb_true_iff in H.
	  destruct H as [H1 H2]. apply IHc1 in H1. apply IHc2 in H2.
  ...
  - intros H. induction H; simpl; try reflexivity; 
    apply andb_true_iff; split; try apply IHno_whilesR1; try apply IHno_whilesR2.
Qed.
...
Proof.
  ...
  - destruct (IHno_whilesR1 st) as [st1 H1].
	...
	+ exists st1. apply E_IfTrue. apply Eqb. apply H1.
	+ exists st2. apply E_IfFalse. apply Eqb. apply H2.
Qed.
...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  | SLoad v :: prog',_ => s_execute st (st v :: stack) prog'
  | SPlus :: prog', x :: y :: stack' => s_execute st ((x + y) :: stack') prog'
  | SMinus :: prog', x :: y :: stack' => s_execute st ((y - x) :: stack') prog'
  | SMult :: prog', x :: y :: stack' => s_execute st ((x * y) :: stack') prog'
...
Proof.
  ...
  - simpl. reflexivity.
  - intros. destruct stack.
    + destruct a; simpl; apply IHp1.
	+ destruct a; destruct stack; simpl; apply IHp1.
Qed.
(** [] *)
...
Proof.
  intros st e. induction e; simpl; try reflexivity; intros stack;
  rewrite execute_app; rewrite IHe1;
  rewrite execute_app; rewrite IHe2; simpl. 
  - rewrite (add_comm (aeval st e1) (aeval st e2)). reflexivity.
  - reflexivity.
  - rewrite (mul_comm (aeval st e1) (aeval st e2)). reflexivity.
Qed.
...
Inductive ceval : com -> state -> result -> state -> Prop :=
  ...
  | E_Seq1 : forall st c1 c2 st' st'' r,
			st =[ c1 ]=> st' / SContinue ->
			st' =[ c2 ]=> st'' / r ->
			st =[ c1 ; c2 ]=> st'' / r
	| E_Seq2 : forall st c1 c2 st',
	 		st =[ c1 ]=> st' / SBreak ->
			st =[ c1 ; c2 ]=> st' / SBreak
	| E_IfTrue : forall st b c1 c2 st' r,
			beval st b = true ->
			st =[ c1 ]=> st' / r ->
			st =[ if b then c1 else c2 end ]=> st' / r
	| E_IfFalse : forall st b c1 c2 st' r,
			beval st b = false ->
			st =[ c2 ]=> st' / r ->
			st =[ if b then c1 else c2 end ]=> st' / r
	| E_WhileTrue : forall st b c st' st'' r,
			beval st b = true ->
			st =[ c ]=> st' / SContinue ->
...
Proof.
  intros. apply E_Seq1 with st'; assumption.
Qed.
...
Theorem seq_stops_on_break : forall c1 c2 st st',
  ...
  st =[ c1 ; c2 ]=> st' / SBreak.
Proof.
  intros. apply E_Seq2; assumption.
Qed.
(** [] *)
...
Proof.
  intros. remember <{while b do c end}> as w eqn:Heqw.
	induction H; try discriminate Heqw; injection Heqw as Heq1 Heq2; subst.
...
Proof.
  intros.
	...
	induction H;
	try (intros; inversion H0; subst; split; reflexivity).
	- intros. inversion H1; subst.
		+ apply IHceval1 in H4. destruct H4 as [H4 _]. subst.
			apply IHceval2 in H8. assumption.
		+ apply IHceval1 in H7. destruct H7 as [_ H7]. discriminate. 
	- intros. inversion H0; subst.
	  + apply IHceval in H3. destruct H3 as [_ H3]. discriminate.
	...
	- intros. inversion H1; subst.
	  + rewrite H8 in H. discriminate.
		+ apply IHceval in H9. assumption.
	- intros. inversion H2; subst.
	  + apply IHceval1 in H6. destruct H6 as [H6 _]. subst.
		  apply IHceval2 in H10. destruct H10 as [H10 _].
			split.
			* assumption.
			* reflexivity.
		+ rewrite H8 in H. discriminate.
		+ apply IHceval1 in H9. destruct H9 as [_ H9]. discriminate.
	- intros. inversion H0; subst.
	  + rewrite H3 in H. discriminate.
		+ split. reflexivity. reflexivity.
		+ rewrite H3 in H. discriminate.
	- intros.  inversion H1; subst.
	  + apply IHceval in H5. destruct H5 as [_ H5]. discriminate.
		+ rewrite H7 in H. discriminate.
		+ apply IHceval in H8. destruct H8 as [H8 _]. split.
		  * assumption.
			* reflexivity.
Qed.
(** [] *)
